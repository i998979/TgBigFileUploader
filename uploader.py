...
async def upload_zip_slices(input_directory, output_directory, password, slice_size="2000m"):
    ...
    for folder_name in os.listdir(input_directory):
        ...
        if os.path.isdir(folder_path):  # Only process directories
            ...
            # Create zip slices for each folder
            zip_folder_in_slices(folder_path, output_directory, password, slice_size)

            # Define the slice pattern for this folder (assuming the zip slices are in the output directory)
            ...
            # Get the user or group entity
            user_id = int(os.getenv('TELEGRAM_GROUP_ID'))  # Replace with your group or user ID (as integer)
            try:
                user = await client.get_entity(user_id)  # Get the user/group entity

                # Upload each zip slice to Telegram
                for slice_path in zip_slices:
                    print(f"Uploading {slice_path} to: {user.id} {user.title}")
            ...
            # Cleanup: Delete all zip slices for this folder after upload
            for slice_path in zip_slices:
                if os.path.exists(slice_path):
